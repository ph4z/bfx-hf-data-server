...
const debug = require('debug')('bfx:hf:data-server:cmds:get-ointerest')
const send = require('../wss/send')

module.exports = async (ds, ws, msg) => {
  const [, exchange, symbol, tf, type, start, end, meta] = msg
  const { db } = ds
  const { openInterest } = db
  ...
  const candles = await openInterest.getInRange([
    ['exchange', '=', exchange],
    ['symbol', '=', symbol],
    ['tf', '=', tf]
  ], {
  ...
  send(ws, ['data.ointerest', exchange, symbol, tf, type, start, end, meta, candles])
