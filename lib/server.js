...
const getAlerts = require('./cmds/get_alerts')
const getLSRatio = require('./cmds/get_lsratio')
const getOInterest = require('./cmds/get_ointerest')
const getFunding = require('./cmds/get_funding')
//const getLiquidations = require('./cmds/get_liquidations')
...
const submitAlert = require('./cmds/submit_alert')
const APIServerClient = require('./hfclient')
...
const COMMANDS = {
  ...
  'get.lsratio': getLSRatio,
  'get.ointerest': getOInterest,
  'get.funding': getFunding,
  //'get.liquidations': getFunding,
...
class DataServer {
  ...
  /**
   * Spawns the websocket client; throws an error if it is already open
   */
  hfclient (wsURL, subMsg) {

    this.wsc = new APIServerClient({url: wsURL})
    this.wsc.ds = this

    // subscribe to channels
    for (let i = 0; i < subMsg.length; i += 1) {
      this.wsc.send(subMsg[i])
    }
