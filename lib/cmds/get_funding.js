...
const debug = require('debug')('bfx:hf:data-server:cmds:get-funding')
...
module.exports = async (ds, ws, msg) => {
  ...
  const candles = await funding.getInRange([
    ['exchange', '=', exchange],
    ['symbol', '=', symbol],
    ['tf', '=', tf]
  ], {
  ...
  send(ws, ['data.funding', exchange, symbol, tf, type, start, end, meta, candles])
