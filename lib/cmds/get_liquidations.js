...
const debug = require('debug')('bfx:hf:data-server:cmds:get-liquidations')
...
module.exports = async (ds, ws, msg) => {
  ...
  const candles = await liquidations.getInRange([
    ['exchange', '=', exchange],
    ['symbol', '=', symbol],
    ['tf', '=', tf]
  ], {
  ...
  send(ws, ['data.liquidations', exchange, symbol, tf, type, start, end, meta, candles])
