...
class DataServer {
  ...
  hfclient (wsURL, subMsg) {
    ...
    //this.wsc = new WS(wsURL)
    this.wsc = new APIServerClient({url: wsURL})

    //this.wsc.on('message', send(this, this.wsc, msg)
    // this.wsc.on('connection', this.onWSConnected.bind(this))
    //this.wss.on('connection', this.onWSConnected.bind(this))
    // subscribe to channel
    this.wsc.send(subMsg)

    debug('websocket dataserver client connected to')
  }
  /**
